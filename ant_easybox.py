...
class AntEasyBoxEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def sample_tasks(self, num_tasks):
        ...
        if self.env_type == 'test':
            box_x_positions = np.linspace(-5, 5, num_tasks)
        else:
            box_x_positions = np.random.uniform(-6, 6, size=(num_tasks,))

        box_y_positions = np.random.uniform(11.999, 12.001, size=(num_tasks,))
