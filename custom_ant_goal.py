...
class CustomMujocoEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, task={}, n_tasks=1, env_type='train', randomize_tasks=True):
        self._task_sets = ["antgoal0","antbrid0","antgath0","antgoal1", "antbrid1","antgath1", "antgoal2", "antbrid2","antgath2", "antgoal3"]
        self.task_order = np.arange(10)
        self._task = task
        self.subtaskid = 0
        self.subtasktypes = ["antgoal","antbridge","antgather","antgoal", "antbridge","antgather", "antgoal", "antbridge","antgather", "antgoal"]
        ...
        xml_path = os.path.join(os.getcwd(), "assets/custom-ant-goal.xml")
    ...
    def combine_subtask(self):
        ...
        for task_no in self.task_order:
            if self._task_sets[task_no][:7] == "antgoal":
                ...
                idx_frontplane = self.model.geom_name2id("antbridge"+str(self._task_sets[task_no][-1])+"_frontplane")
    ...
    def set_coins(self, start_position, coin1, coin2, ith_antgather):
        ...
        first_coin_x_position = np.random.uniform(-4.5, 4.5)
